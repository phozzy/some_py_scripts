...
def spawn_brackets(accout, itout):
    ...
    # return acc
    return reduce(lambda acc, it: ((lambda open_brackets, clos_brackets: acc + ((clos_brackets < open_brackets) and [it + ')'] or []) + ((open_brackets < itout) and [it + '('] or [])) (it.count('('), it.count(')'))), accout, [])
